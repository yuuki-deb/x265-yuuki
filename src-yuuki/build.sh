...
for distribution in $distributions; do
  ...
  docker build \
    ...
    --build-arg rev=$rev \
    -f $dockerfile -t x265-yuuki:$distribution .

  build_dir=`docker inspect x265-yuuki:$distribution | jq -r '.[0].GraphDriver.Data.UpperDir'`/build

  [[ $(ls $build_dir/reg/x265-yuuki_*.deb | head -1) =~ $reg ]]
